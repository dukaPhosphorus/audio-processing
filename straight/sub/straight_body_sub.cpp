		 ...
		 XBOOL msg_flag)
{
    ...
    if (msg_flag == XTRUE) fprintf(stderr, "spectrogram reshaping done\n");
		       ...
		       XBOOL msg_flag)
{
	...
	for (k = 0; k < xctbl->length; k++) xctbl->data[k] = (fr40 / f0x->data[ii] * rt->data[k] -0.5 * deltat0->data[ii] / f0x->data[ii] * rt2->data[k] * SQUARE(fr40 / f0x->data[ii])) + (double)(ii + 1) * shiftm / 1000.0;
	if ((xs = interp1lq(txc, xc, xctbl)) == NODATA) {
	    fprintf(stderr, "aperiodicpart4: Failed (1)\n");
	    return NODATA;
	}
	...
	if ((apvv = interp1q(gg2, gfg2, fxa)) == NODATA) {
	    fprintf(stderr, "aperiodicpart4: Failed (2)\n");
	    return NODATA;
	}
	if ((dpvv = interp1q(dd2, dfd2, fxa)) == NODATA) {
	    fprintf(stderr, "aperiodicpart4: Failed (3)\n");
	    return NODATA;
	}
	...
	/*
	if (f0l->data[iix] > 300.0) {
	    fprintf(stderr, "F0 = %f\n", f0l->data[iix]);
	    for (k = 0; k < dpvv->length; k++)
		printf("%f %f\n", (double)k / (double)(dpvv->length - 1) * 8000.0, dpvv->data[k]);
	    //	printf("%f %f\n", (double)k / (double)(apvv->length - 1) * 8000.0, apvv->data[k]);
	    exit(1);
	    // memory free
	    xdvfree(gg2);	xdvfree(gfg2);		xdvfree(apvv);
	    for (k = 0, len = 0; k < fftl2 - 1; k++, len++) {
		// frequency [Hz]
		gg->data[len] = (double)(k + 1) / (double)fftl * fs;
		// power spectrum [dB]
		gfg->data[len] = sms->data[k + 1];
	    ...
	    // memory allocation
	    gg2 = xdvalloc(len + 2);	gfg2 = xdvalloc(len + 2);
	    gg2->data[0] = 0.0;		gfg2->data[0] = sms->data[0];
	    gg2->data[len + 1] = fs / 2.0 * f0x->data[ii] / fr40;
	    gfg2->data[len + 1] = sms->data[hfftl];
	    // frequency * F0 / 40 [Hz]
	    for (k = 0; k < len; k++) {
	    ...
	    // interpolation (extract envelope 0--Nyquist * 40 / F0 [Hz])
	    if ((apvv = interp1q(gg2, gfg2, fxa)) == NODATA) {
		fprintf(stderr, "aperiodicpart4: Failed (2)\n");
		return NODATA;
	    }
	    for (k = 0; k < apvv->length; k++)
		printf("%f %f\n", (double)k / (double)(apvv->length - 1) * 8000.0, apvv->data[k]);
	    exit(1);
	}
	*/
    ...
    if (msg_flag == XTRUE)
	fprintf(stderr, "extraction of MBE-source information done\n");
...
DVECTOR interp1l(DVECTOR x, DVECTOR y, DVECTOR xi)
{
    ...
    /* error check */
    if (ki2 != xo->length) {
	fprintf(stderr, "Failed: Interpolation\n");
	return NODATA;
    }
	      ...
	      long *ansidx)
{
    ...
    *len = lidx - fidx + 1;
    if (*len < 2) {
	fprintf(stderr, "Erorr: binary search\n");
	exit(1);
    }
    if (*len > 2) {
	idx = *len / 2 + fidx;
...
DVECTOR xdvread_cf0file(char *cf0file, double *f0shift)
{
	...
	fprintf(stderr, "Error xdvread_cf0file: file format\n");
	exit(1);
    }
...
DVECTOR xget_interp_f0(DVECTOR f0l, double f0shift, double shift)
{
    ...
    if (f0shift <= 0.0) {
	fprintf(stderr, "Error F0shift > 0.0[ms]\n");
	exit(1);
    }
    coef = shift / f0shift;
    p = 0.0;	a = 0.0;	i = 0;
