...
//    fprintf(stderr, "Frame length = %f\n", (double)framel / fs * 1000.0);
//    fprintf(stderr, "shift length = %f\n", (double)shiftl / fs * 1000.0);
    // Hamming window
...
double *dalloc(int cc)
{
  ...
  if( (ptr=(double *)malloc(cc*sizeof(double))) == NULL){
    fprintf(stderr,"メモリーallocに失敗しました。\n");return(0);}
  return(ptr);
}
		      ...
		      XBOOL inv_flag)
{
    ...
    for (k = 0; k < cepg->row; k++) {
	cep = xdmextractrow(cepg, k);
	mcep = xcep2mpmcep(cep, order, fftl, power_flag, inv_flag);
	dmcopyrow(mcepg, k, mcep);
	xdvfree(cep);
	xdvfree(mcep);
    }
...
DMATRIX xget_fftmat(DMATRIX mat, long fftl, long order, XBOOL inv_flag)
{
    ...
    // error check
    if (mat->col > hfftl) {
	fprintf(stderr, "fft length is too short %ld\n", fftl);
	exit(1);
    }
