		 ...
		 long dn)
{
	...
	if ((filter = cheby(8, 0.05, 0.8 / (double)dn)) == NODATA) {
	    fprintf(stderr, "Failed: Chebyshev Filter\n");
	    return NODATA;
	}
		    ...
		    DVECTOR x)	/* input data */
{
    ...
    if (a->data[0] != 1.0) {
	fprintf(stderr, "normalize filter-coefficient\n");
	dvscoper(b, "/", a->data[0]);
	dvscoper(a, "/", a->data[0]);
	       ...
	       double endf)
{
	...
	fprintf(stderr, "0 <= freq <= 1, freq = %f, %f\n", startf, endf);
	exit(1);
    } else if (startf > endf) {
	fprintf(stderr, "start-freq(%f) < end-freq(%f)\n", startf, endf);
	exit(1);
    }
		   ...
		   XBOOL allparam_flag)	/* XFALSE*/
{
	...
	if ((wa = interp1q(xo, wo, xi)) == NODATA) {
	    fprintf(stderr, "Error: refineF02\n");
	    return NODATA;
	}
	wal = wa->length;
	/* memory free */
		   ...
		   XBOOL allparam_flag)	/* XFALSE*/
{
    ...
    if (sml < 2) {
	fprintf(stderr, "Shift is too long\n");
	exit(1);
    }
    /* bias due to filtering */
    smb = (sml - 1) / 2;
		    ...
		    long f0jmp)
{
    ...
    len = MIN(f0v->col, f0raw->length);
    idxv = xalloc((int)f0v->row, long);
    vrvvec = xalloc((int)f0v->row, double);

    fprintf(fp, "# FrameShift=%ld\n", f0shiftl);
    for (k = 0; k < len; k += f0jmp) {
