...
void ss_waveampcheck(DVECTOR wav)
{
    ...
    if (value >= 32000.0) {
	fprintf(stderr, "straight synth: power is too big: %f\n", value);
	fprintf(stderr, "straight synth: execute normalization\n");
		   ...
		   double tsconv)	/* time axis expansion coefficient */
{
	...
	fprintf(stderr, "Error: makeimaptm\n");
	return NODATA;
    }
    ...
    fprintf(stderr, "time scale function done\n");
		   ...
		   double smfsht)	/* frame shift of spectrum movement calculation (ms) */
{
	...
	fprintf(stderr, "Error: spcmovedb\n");
	return NODATA;
    }
