...
//    DVECTORS filter = NODATA;	/* coefficients for filter */
    ...
    */
    ...
    // modified by Tomoki Toda on Mar. 30, 2010
    if (nflr_flag == XFALSE) {
	...
	xt = xdvrandn(xh->length + framel / 2 + framel);
	dvscoper(xt, "*", rmsp / 4000.0);
	dvpaste(xt, xh, framel / 2, xh->length, 1);
    }
    /* memory free */
    xdvfree(xh);
    ...
    if (msg_flag == XTRUE) fprintf(stderr, "frequency smoothing done\n");
	...
	if (msg_flag == XTRUE) fprintf(stderr, "temporal smoothing done\n");
    }
	...
	*/
	...
	if (msg_flag == XTRUE) fprintf(stderr, "unvoiced part shaping done\n");
